...
src_install() {
	gnome2_src_install
	python_replicate_script "${ED}/usr/bin/page-shell-extension-tool"
	python_replicate_script "${ED}/usr/bin/page-shell-perf-tool"

	# Required for gnome-shell on hardened/PaX, bug #398941
	# Future-proof for >=spidermonkey-1.8.7 following polkit's example
	if has_version '<dev-lang/spidermonkey-1.8.7'; then
		pax-mark mr "${ED}usr/bin/page-shell"{,-extension-prefs}
	elif has_version '>=dev-lang/spidermonkey-1.8.7[jit]'; then
		pax-mark m "${ED}usr/bin/page-shell"{,-extension-prefs}
	...
	# PaX EMUTRAMP need to be on
		...
		pax-mark E "${ED}usr/bin/page-shell"{,-extension-prefs}
	else
		pax-mark m "${ED}usr/bin/page-shell"{,-extension-prefs}
	fi
}
